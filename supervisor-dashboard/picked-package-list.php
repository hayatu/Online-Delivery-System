...
<?php
	...
	pack.city_name,pack.createdate,pack.first_name as s_fname,pack.last_name as s_lname,pack.mobile as s_phone,pack.vehicle_id,pack.departure_time,
	pack.c_first_name, pack.c_last_name,pack.c_mobile,
	tbl_employee_info.first_name as em_fn, 
	tbl_employee_info.last_name as em_ln,tbl_employee_info.mobile as em_phone, tbl_status_info.status, 
	...
	."where pack.deleteflag = 0 and assign_status = 1 and pickedby = 1 order by pack.weight, tbl_employee_info.first_name ASC");	  
			...
			<tbody>
				<?php
					...
					<tr>	
						...
						<td><?php if($rs_package[$i]["em_fn"]." ".$rs_package[$i]["em_ln"]=='') {echo $rs_package[$i]["pickedby"]; }else {echo $rs_package[$i]["em_fn"]." ".$rs_package[$i]["em_ln"]; }?></td>						
						<td><?php echo $rs_package[$i]["em_phone"]; ?></td>
						<td><?php echo $rs_package[$i]["made_model"]; ?></td>
						<td><?php echo $rs_package[$i]["plat_number"]; ?></td>
						<td><?php echo $rs_package[$i]["departure_time"]; ?></td>
						<td><?php echo $rs_package[$i]["is_fragile"]; ?></td> 
						<td><?php echo $rs_package[$i]["zone_name"]; ?></td>	
						<td><?php echo $rs_package[$i]["city_name"]; ?></td>
						<td><?php echo $rs_package[$i]["s_fname"]." ".$rs_package[$i]["s_lname"]; ?></td>
                         <td><?php echo $rs_package[$i]["s_phone"]; ?></td>							
