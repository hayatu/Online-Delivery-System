...
<?php
	...
	$conn = db_connect();	
	    ...
	    $query1 = sprintf("select * from tbl_package_info  where zone_name =%s and deleteflag = 0 and assign_status = 0 ORDER BY package_id DESC",
	  GetSQLValueString($_REQUEST["zone-name"], "text"));
			...
			<tbody>
				<?php
					...
					<tr class="alert-info">
                       ...
                       <td><a href="package-update-form.php?package-id=<?php echo $rs_package[$i]["package_id"]; ?>&&customer-id=<?php echo $rs_package[$i]["customer_id"]; ?>" class="btn btn-sm" role="button"><i class="fas fa-edit"></i></a></td>							
						...
						<!--<td><input type="button" value="Print" onclick="var w = window.open('print-package.php?package-id=<?php echo $rs_package[$i]["package_id"]; ?>','mywin'); w.print();"/></td>-->
						 <td><a href="print-package.php?package-id=<?php echo $rs_package[$i]["package_id"]; ?>" class="btn btn-sm" role="button"><i class="fas fa-print"></i></a></td>
