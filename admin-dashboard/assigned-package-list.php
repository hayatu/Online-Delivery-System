...
<?php
	...
	$conn = db_connect();	
       ...
       $query1 = sprintf("select pack.package_id, pack.emp_id, pack.track_num, pack.weight,pack.quantity,pack.is_fragile, pack.current_loc,
					...
					tbl_employee_info.last_name as em_ln,tbl_employee_info.bgcolor as bgcolor,tbl_employee_info.vehicle_id as empvehicle_id,tbl_status_info.status, 
			...
			<tbody>
				<?php
					...
					<tr style= "background-color:<?php echo $rs_package[$i]["bgcolor"]; ?>">	
						...
						<td><?php 
					  		$from = urlencode($rs_package[$i]["current_loc"]);
							$to = urlencode($rs_package[$i]["destination"]);
							$data = file_get_contents("https://maps.googleapis.com/maps/api/distancematrix/json?origins=$from&destinations=$to&language=en-EN&sensor=false&key=AIzaSyBIIGKlQtBEwWgM5DBjNebNWFhraJuNNpQ");
							...
							$mi=round($distance/1000/1.609344,1);
							$ntime = round($time/60);
                             echo " ".$mi." mile";
							  //echo " ".$km." Km" ." ".$ntime." minutes";
							//echo $km;
					  ?></td>						
					   ...
					   <td><a href="assigned-package-list-details.php?emp-id=<?php echo $rs_package[$i]["emp_id"]; ?>" class="btn btn-sm" role="button"><i class="fas fa-info-circle"></i></a></td>
						<td><a href="employee-vehicle-update-form.php?emp-id=<?php echo $rs_package[$i]["empvehicle_id"]; ?>" class="btn btn-sm" role="button"><i class="fas fa-palette"></i></a></td>
						<td>						
